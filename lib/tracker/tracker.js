...
Peer.prototype = {
	...
	timedOut: function(n) {
		return n - this.lastAction > announceInterval * 2;
	},
...
File.prototype = {
	addPeer: function(peer, event) {
		...
		var n = now();
		if (this.seeders + this.leechers < this.peerList.length / 2 && this.peerList.length > 10 || (n - this.lastCompact) > announceInterval * 2) {
			newPeerList = [];
			var i = 0;
	...
	writePeers: function(b, count, peer_id) {
		...
		var sampledPeers = this.peerList; //floyd(this.peerList, m);
	...
	getPeers: function(count, peer_id) {
		...
		for (var i = sampledPeers.length-1; i >= 0 && c < count; i--) {
			var p = sampledPeers[i];
			var includePeer = (p !== undefined && p !== null);
			if (p && peer_id && p.peer_id && (peer_id+'').trim()===(p.peer_id+'').trim()) {
				includePeer = false;
			}
			if (includePeer) {
...
function Tracker(announceInterval) {
	if (!(this instanceof Tracker))
		return new Tracker(announceInterval);

	if (typeof announceInterval !== 'undefined' && announceInterval !== null) {
		this.announceInterval = announceInterval;
		exports.announceInterval = announceInterval;
	}
	this.files = {};
}
